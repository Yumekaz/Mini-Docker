...
def create_parser() -> argparse.ArgumentParser:
    ...
    run_parser.add_argument("--no-overlay", action="store_true",
                           help="Use chroot instead of OverlayFS")
    run_parser.add_argument("--cpu", "-c", type=int, 
                           help="CPU limit percentage (1-100)")
    run_parser.add_argument("--memory", "-m", 
                           help="Memory limit (e.g., 100M, 1G)")
    run_parser.add_argument("--pids", type=int, 
                           help="Max number of processes")
    run_parser.add_argument("--pids-limit", type=int, 
                           help="Max number of processes (alias for --pids)")
    run_parser.add_argument("--env", "-e", action="append", default=[],
                           help="Set environment variable (KEY=VALUE)")
    run_parser.add_argument("--volume", "-V", action="append", default=[],
                           help="Bind mount a volume (host:container)")
    run_parser.add_argument("--workdir", "-w", default="/", 
                           help="Working directory inside container")
    run_parser.add_argument("--user", "-u", 
                           help="User to run as (user or uid:gid)")
    run_parser.add_argument("--net", action="store_true",
                           help="Enable networking")
    run_parser.add_argument("--pod", help="Pod to join")
    run_parser.add_argument("--rootless", action="store_true",
    ...
    run_parser.add_argument("--tty", "-t", action="store_true",
    ...
    oci_parser.add_argument("--detach", "-d", action="store_true",
                           help="Run container in background")
    oci_parser.add_argument("--rootless", action="store_true",
    ...
    exec_parser.add_argument("--env", "-e", action="append", default=[],
                            help="Set environment variable (KEY=VALUE)")
    exec_parser.add_argument("--user", "-u", 
                            help="User to run as (user or uid:gid)")
    ...
    ps_parser.add_argument("--all", "-a", action="store_true",
    ...
    ps_parser.add_argument("--format", "-f", choices=["table", "json"],
    ...
    logs_parser.add_argument("--timestamps", "-t", action="store_true",
    ...
    inspect_parser.add_argument("container", nargs="+", 
                               help="Container ID(s) or name(s)")
    inspect_parser.add_argument("--format", "-f", choices=["json", "yaml"],
    ...
    stop_parser.add_argument("container", nargs="+", 
                            help="Container ID(s) or name(s)")
    stop_parser.add_argument("--time", "-t", type=int, default=10,
    ...
    rm_parser.add_argument("container", nargs="+", 
                          help="Container ID(s) or name(s)")
    rm_parser.add_argument("--force", "-f", action="store_true",
                          help="Force removal of running container")
    rm_parser.add_argument("--volumes", "-v", action="store_true",
    ...
    pod_create.add_argument("--net", action="store_true",
                           help="Enable networking for pod")
    ...
    pod_list.add_argument("--quiet", "-q", action="store_true",
                         help="Only display pod IDs")
    pod_list.add_argument("--format", "-f", choices=["table", "json"],
                         default="table", help="Output format")
    ...
    pod_rm.add_argument("--force", "-f", action="store_true",
                       help="Force removal of running pod")
    
    # pod inspect
    pod_inspect = pod_subparsers.add_parser("inspect", help="Inspect a pod")
    pod_inspect.add_argument("pod", help="Pod ID or name")
    pod_inspect.add_argument("--format", "-f", choices=["json", "yaml"],
                            default="json", help="Output format")
    ...
    build_parser.add_argument("--file", "-f", help="Build file name", 
                             default="Imagefile")
    ...
    images_parser.add_argument("--quiet", "-q", action="store_true",
                              help="Only display image IDs")
    images_parser.add_argument("--format", "-f", choices=["table", "json"],
                              default="table", help="Output format")
    ...
    rmi_parser.add_argument("--force", "-f", action="store_true",
                           help="Force removal")
    ...
    info_parser.add_argument("--format", "-f", choices=["text", "json"],
    ...
    version_parser.add_argument("--format", "-f", choices=["text", "json"],
                               default="text", help="Output format")
    ...
    cleanup_parser = subparsers.add_parser("cleanup", 
                                           help="Clean up unused resources")
    cleanup_parser.add_argument("--all", "-a", action="store_true",
                               help="Remove all unused data")
    cleanup_parser.add_argument("--containers", action="store_true",
                               help="Remove stopped containers")
    cleanup_parser.add_argument("--images", action="store_true",
...
def cmd_ps(args: argparse.Namespace) -> int:
    ...
    if args.quiet:
        ...
        # Table output
        print(f"{'CONTAINER ID':<14} {'NAME':<20} {'STATUS':<12} {'COMMAND':<30} {'CREATED'}")
...
def cmd_rm(args: argparse.Namespace) -> int:
    ...
    for container_id in args.container:
        try:
            container.remove(container_id, force=args.force, 
        ...
        for c in all_containers:
            if c.pod_id:
                if pod_filter and not c.pod_id.startswith(pod_filter):
                    continue
                print(f"{c.id[:12]:<14} {c.pod_id[:12]:<14} {(c.name or '')[:20]:<20} {c.status}")
        return 0
