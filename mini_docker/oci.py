...
class OCIProcess:
    ...
    env: List[str] = field(default_factory=lambda: ["PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"])
    cwd: str = "/"
...
def parse_oci_config(data: Dict) -> OCIConfig:
    ...
    if "linux" in data:
        ...
        for ns in linux.get("namespaces", []):
            namespaces.append(OCINamespace(
                type=ns.get("type", ""),
                path=ns.get("path"),
            ))
...
def oci_to_container_config(
    ...
    if oci_config.process.capabilities:
        ...
        cap_set = (
            oci_config.process.capabilities.get("bounding") or
            oci_config.process.capabilities.get("permitted") or
            []
        )
        caps = cap_set
