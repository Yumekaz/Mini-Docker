...
class Container:
    ...
    def start(self, container_id: str) -> int:
        ...
        if not config.rootless and not check_root():
            raise ContainerError("Root privileges required (use --rootless for unprivileged mode)")
        ...
        if pid == 0:
            ...
            if config.rootless:
                try:
                    ...
                    print(f"Warning: Failed to setup user namespace: {e}", file=sys.stderr)
    ...
    def _run_container(self, config: ContainerConfig, sync_read: Optional[int] = None, sync_write: Optional[int] = None) -> None:
        """
        ...
        try:
            ...
            # Create namespaces
            namespaces_to_create = [ns for ns in config.namespaces if ns not in pod_ns_paths]
            ...
            for ns_type, ns_path in pod_ns_paths.items():
                try:
                    ...
                    logger.write(f"Warning: Failed to enter pod namespace {ns_type}: {e}\n")
            ...
            if config.use_overlay:
                try:
                    ...
                    logger.write(f"Warning: Overlayfs setup failed ({e}), falling back to direct chroot\n")
    ...
    def remove(self, container_id: str, force: bool = False, remove_volumes: bool = False) -> bool:
        """
        Remove a container.
